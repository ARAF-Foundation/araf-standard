...
Governance weaknesses compound. ARAF incorporates four structural multipliers:
...
- Political Cascade: D5 at or above 4 and D3 at or above 3, with single-provider dependency and government-adjacent customer concentration. Government designation events can convert customer concentration into immediate revenue disruption where contract infrastructure does not address the cascade mechanism.
...
The GCI operates as a divisor on the base GBI dimensional score:

`Effective Dimension Score = GBI Dimension Score / GCI Dimension Score`

A GCI of 0.5 applied to a GBI dimensional score of 2.0 produces an effective score of 4.0, correctly reflecting that poor operational coherence significantly worsens the effective governance posture. Good governance design does not compensate for poor operational execution.
...
The standard is currently maintained under a founder-led governance model with governance authority held by the Institute. Five principles guide maintenance: purpose primacy, version discipline, public transparency, methodological consistency, and institutional usability. The governance model is designed to scale with the ecosystem: from the current founder-led stage, through an advisory stage incorporating institutional and technical contributors, toward a multi-stakeholder governance model as the certification ecosystem reaches institutional scale. Transition is triggered by ecosystem maturity, not calendar time.

### Who conducts ARAF assessments?

Assessments are conducted by accredited assessors. Accreditation requires demonstrated governance assessment competence, ARAF methodology proficiency, structural independence from assessed organisations, and submission to quality assurance review. The standard maintainer in its advisory services capacity (Venture Bench Pty Ltd) operates under identical independence requirements. Self-certification does not satisfy the ARAF independence requirement.
